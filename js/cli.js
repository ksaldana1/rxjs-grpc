...
function transformTypeScriptSource(source) {
    ...
    source = source.replace(/request:.*(?=\))/gm, '$&, metadata?: any, callOptions?: object');
    return source;
}
...
function constructorsToInterfaces(ast) {
    ast.find(jscodeshift.FunctionDeclaration).forEach(function (path) {
        path.node.comments.forEach(function (comment) {
...
function getNamepaceDeclarations(ast) {
    return ast
        ...
        return path.node.comments.some(function (comment) {
